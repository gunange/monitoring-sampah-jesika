import { dataEnv } from '@/components/dashboard/admin/config'
...
export class Controller {
  get collection() {
    return `${dataEnv.path_api}/software-komputer`
  }
  get time() {
    return new TimeApp()
  }
  get storage() {
    return userStorage().software_komputer
  }
}
export class Cruds extends Controller {
  ...
  async up_item(data: any, id: Number, status: Number) {
    if (status === 201 || status === 200) {
      const index = this.storage.data.findIndex((dataItem) => dataItem.id === id)
      if (index !== -1) {
  ...
  async up(body: any): Promise<void> {
    const { data, status } = await patch(`${this.collection}/${this.uid}`, body)
  ...
  async switchStatus(body: any): Promise<void> {
    const { data, status } = await patch(`${this.collection}/switch-status/${this.uid}`, body)
