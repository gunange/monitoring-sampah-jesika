...
const open = async (act, uid) => {
  ...
  if (act == 'add') {
    ref_form.value.setFieldValue('komputer_id', uid)
  } else if (act == 'up') {
    main.setUid(uid)
      ...
      <VeeForm @submit="onSave" :initial-values="form" ref="ref_form">
        <div class="text-xs grid grid-cols-1 gap-4">
          <div class="form" v-if="modal.act === 'add'">
            <VeeField v-slot="{ field }" name="nama" rules="required" v-model="form.nama">
              <label>
                <span>Nama</span>
              ...
              <v-select
                ...
                :reduce="(x) => x.label"
                v-model="form.nama"
                :options="atribut.software.items"
                :appendToBody="true"
                :loading="atribut.software.data.load"
                @open="atribut.software.init()"
              />
            </VeeField>
          </div>
          <div class="form">
            <VeeField v-slot="{ field }" name="versi" rules="required" v-model="form.versi">
              <label>
                <span>Versi</span>
                <span class="text-red-500"> * <VeeErrorMessage name="versi" /> </span>
              </label>

              <InputText
                v-bind="field"
                placeholder="Masukan versi software"
                class="text-xs"
                autocomplete="off"
              />
            </VeeField>
