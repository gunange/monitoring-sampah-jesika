<script setup>
	...
	const open = async (act, item) => {
		ref_pdf.value = null;
		form.value = item;
		const { data, status } = await main.getFileInfo(form.value.lampiran);
		if (status == 200) {
			fileInfo.value = data;
		}
			...
			<template #default>
				<div class="" v-if="fileInfo?.mime_type">
					<div class="" v-if="imageType.includes(fileInfo?.mime_type)">
						<Image
							:src="main.urlStorage + '/' + form.lampiran"
							alt="Image"
							imageClass="w-full"
						/>
					</div>
					<div class="w-full" v-else-if="pdfType.includes(fileInfo?.mime_type)">
						<PdfVue :url="main.urlStorage + '/' + form.lampiran" ref="ref_pdf" />
					</div>
					<div
				...
				<div class="text-red-400 text-center" v-else>
					...
					<p>File Tidak Ditemukan (Tidak ada atau sudah dihapus)</p>
				</div>
			</template>
		</Dialog>
	</main>
